...
def get_sum_zero_pairs(input_list):
    ...
    for num1 in input_list:
        for num2 in input_list[index:]:
            if num1 + num2 == 0:
                pair_zero = [num1, num2]
                pair_zero.sort()
                if pair_zero not in sum_zero_list:
                    sum_zero_list.append(pair_zero)
                index = index + 1
...
def encode(phrase):
    ...
    """
    replace_characters = {'e': 'p', 'a': 'd', 't': 'o', 'i': 'u'}
    chars_list = []

    for char in phrase:
        if char in replace_characters:
            chars_list.append(replace_characters[char])
        else:
            chars_list.append(char)

    string_replace = ''.join(chars_list)
...
def sort_by_word_length(words):
    ...
    for word in words:
        if len(word) not in word_length_tally:
            word_length_tally[len(word)] = [word]
        else:
            word_length_tally[len(word)] = word_length_tally[len(word)] + [word]

    tup_list = word_length_tally.items()
    ascend_list = sorted(tup_list)
    return ascend_list
...
def get_pirate_talk(phrase):
    ...
    """
    Pirate_translations = {'sir': 'matey', 'hotel': 'fleabag inn', 'student': 'swabbie',
    ...
    for word in list_phrase:
        if word in Pirate_translations:
            translated_word_list.append(Pirate_translations[word])
        else:
            translated_word_list.append(word)

    translated_string = ' '.join(translated_word_list)
...
def adv_alpha_sort_by_word_length(words):
    ...
    ascending_list = sorted(tuple_list)

    for index in range(len(ascending_list)):
        ascending_list[index][1].sort()
